...
function getBattles($conn) {
	$sql = "SELECT battles.id AS battle_id, user_id, wager, opponent_score, username FROM battles INNER JOIN users ON users.id = battles.user_id WHERE battles.project_id = '".$_SESSION['userData']['project_id']."' AND active = '1' GROUP BY username ORDER BY wager DESC";
	$result = $conn->query($sql);
