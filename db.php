...
function evaluateText($text){
	...
	if(isset($_SESSION['userData']['project_id'])){
		if($_SESSION['userData']['project_id'] == 2){
			...
			$replacements['Pull Smoke Bomb Pin'] = 'Pull Out Whip';
			$replacements['Explosion'] = 'Spank';
	...
	if($type == "opponent"){
		...
		if ($conn->query($sql) === TRUE) {
			...
			if($_SESSION['userData']['score'] > $opponent_score){
				...
				logBattleCredit($conn, $user_id, $battle_id, $wager);
				logBattleDebit($conn, $opponent_id, $battle_id, $wager);
				...
				logBattleCredit($conn, $opponent_id, $battle_id, $wager);
				logBattleDebit($conn, $user_id, $battle_id, $wager);
...
function getProjects($conn) {
	$sql = "SELECT id, name FROM projects WHERE name != 'Oculus Lounge' && name != 'Dread City' && name != 'Filthy Mermaid'";
	$result = $conn->query($sql);
	$projects = array();
	if ($result->num_rows > 0) {
	  // output data of each row
...
function checkDefaultSoldiers($conn){
	$sql = "SELECT title, soldiers.id AS soldier_id FROM soldiers INNER JOIN users ON users.id = soldiers.user_id WHERE user_id = '".$_SESSION['userData']['user_id']."' AND project_id = '".$_SESSION['userData']['project_id']."' AND rank = 'Grunt'";
	$result = $conn->query($sql);
...
function getSoldiers($conn, $active, $filterby="", $all=false){
	...
	$sql = "SELECT name, asset_name, ipfs, rank, armor, gear, level, username, deceased, active, soldiers.id AS soldier_id FROM soldiers INNER JOIN users ON users.id = soldiers.user_id WHERE ".$user_clause.$active_clause.$filterby." AND project_id = '".$_SESSION['userData']['project_id']."' ORDER BY deceased, title, soldiers.id";
	$result = $conn->query($sql);
...
// Log a specific user credit for a battle
function logBattleCredit($conn, $user_id, $battle_id, $amount) {
	$sql = "INSERT INTO transactions (type, user_id, battle_id, amount, project_id)
	VALUES ('credit', '".$user_id."', '".$battle_id."', '".$amount."', '".$_SESSION['userData']['project_id']."')";
...
// Log a specific user debit for a battle
function logBattleDebit($conn, $user_id, $battle_id, $amount) {
	$sql = "INSERT INTO transactions (type, user_id, battle_id, amount, project_id)
	VALUES ('debit', '".$user_id."', '".$battle_id."', '".$amount."', '".$_SESSION['userData']['project_id']."')";
...
function transactionHistory($conn) {
	if(isset($_SESSION['userData']['user_id'])){
		...
		$loss = "<img class='icon' src='icons/skull.png'/>";
		while($row = $result->fetch_assoc()) {
